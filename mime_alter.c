...
int AM_init( void )
{
	...
	glb.force_for_bad_html=0;	/* (Orig: By default don't try append into broken HTML), Force to 1*/
	...
	glb.pretext_b64=NULL;
	glb.pretext_b64_type=AM_DISCLAIMER_TYPE_NONE;
	*/
#endif
...
//	bodypos = htmlpos = tagpos = -1;
	...
	while (FFGET_fgets(line, AM_1K_BUFFER_SIZE, f))
	{
		...
		if (glb.pretext_insert == 1) {
			//prebody =  strstr(lline,"<NULL");
			prebody = NULL;
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<body");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<html");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<div");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<p");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<head");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<style");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<h1");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<h2");
			if ((!prebody)&&(dd->html_inserted==0)) prebody =  strstr(lline,"<h3");
			if (prebody) {
				char *pt;
				pt = strchr(prebody,'>');
		...
		if (prebody != NULL)
		{
			...
			//DAM LOGGER_log("%s:%d:AM_add_disclaimer_insert_html:DEBUG: line = %s",FL,line);
			//DAM LOGGER_log("%s:%d:AM_add_disclaimer_insert_html:DEBUG: lline = %s",FL,lline);
			...
			//tmpbody = prebody;
			//DAM LOGGER_log("%s:%d:AM_add_disclaimer_insert_html:DEBUG: tmpbody = %s",FL,tmpbody);
			*tmpbody = '\0';
			//DAM LOGGER_log("%s:%d:AM_add_disclaimer_insert_html:DEBUG: tmpbody = %s",FL,tmpbody);
...
int AM_insert_disclaimer_into_segment( FFGET_FILE *f, FILE *newf, struct AM_disclaimer_details *dd )
{
		...
		)
	{
		...
		{
			LOGGER_log("WARNING: ~2548  Could not insert HTML disclaimer into email");
			LOGGER_log("%f",result);
		} else {
			dd->html_inserted = 1;
			insert_success = 1;
